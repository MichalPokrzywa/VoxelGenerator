...
{
    public static void LoopThroughTheBlocks(ChunkData chunkData, Action<int, int, int> actionPerform)
    {
        for(int index = 0; index < chunkData.blocks.Length; index++)
        {
            var position = GetPosiitonFromIndex(chunkData, index);
            actionPerform(position.x, position.y, position.z);
        }
    }
    //convert a 1d array index to a 3d arrayindex
    static Vector3Int GetPosiitonFromIndex(ChunkData chunkData, int index)
    {
    ...
    static int GetIndexFromPosition(ChunkData chunkData, int localPositionX, int localPositionY, int localPositionZ)
    {
        return localPositionX + chunkData.chunkSize * localPositionY + chunkData.chunkHeight * chunkData.chunkSize * localPositionZ;
    }
    ...
    static bool InRangeHeight(ChunkData chunkData, int yCoordinates)
    {
        if (yCoordinates < 0 || yCoordinates >= chunkData.chunkHeight)
            return false;
        return true;
    }
    ...
    internal static Vector3Int ChunkPositionFromCoordinates(World world, int x, int y, int z)
    {
